...
def _extract_observation_tensor(observation: gym_env_pb2.TensorValue) -> gym_env_pb2.Tensor:
    if observation.WhichOneof("kind") != "tensor":
        raise RuntimeError("pid static v2 demo expects tensor observation")
...
def _send_step(
    ...
    if len(reply.step.reward) != 1 or len(reply.step.terminated) != 1 or len(reply.step.truncated) != 1:
        raise RuntimeError("pid static v2 demo expects single-env repeated scalars")
    return {
...
def _draw_overlay(frame: np.ndarray, info: dict[str, object], metrics: dict[str, float] | None) -> None:
    ...
    lines = [
        ...
        f"yaw pid=({metrics['yaw_p']:.2f}, {metrics['yaw_i']:.2f}, {metrics['yaw_d']:.2f})",
        f"pit pid=({metrics['pitch_p']:.2f}, {metrics['pitch_i']:.2f}, {metrics['pitch_d']:.2f})",
...
def main() -> None:
    ...
    parser.add_argument("--align-threshold-deg", type=float, default=0.2)
    parser.add_argument("--plane-threshold", type=float, default=0.01)
    parser.add_argument("--max-steps", type=int, default=2000)
    parser.add_argument("--ff-gain", type=float, default=0.35)
    parser.add_argument("--yaw-kp", type=float, default=1.1)
    parser.add_argument("--yaw-ki", type=float, default=0.08)
    parser.add_argument("--yaw-kd", type=float, default=0.18)
    parser.add_argument("--pitch-kp", type=float, default=1.1)
    parser.add_argument("--pitch-ki", type=float, default=0.08)
    parser.add_argument("--pitch-kd", type=float, default=0.18)
    ...
    with grpc.insecure_channel(args.server_addr) as channel:
        ...
        cv2.namedWindow("Oracle PID Auto Aim V2", cv2.WINDOW_NORMAL)
        ...
        try:
            for step_idx in range(args.max_steps):
                ...
                if step_idx % FRAME_SKIP == 0:
                    ...
                    cv2.imshow("Oracle PID Auto Aim V2", frame)
                    key = cv2.waitKey(1)
                    if (key & 0xFF) == 27:
                        break
